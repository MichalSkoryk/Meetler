...
public class AvailabilityTemplateResolver {
  ...
  public List<ResolvedAvailabilityWindowResponse> resolve(
    ...
    List<AvailabilityTemplateBlock> oneOffBlocks =
        blockRepository.findByTemplateOverlappingRange(template, from, to);

    List<ResolvedAvailabilityWindowResponse> oneOffWindows =
        oneOffBlocks.stream().map(block -> toResolvedOneOffWindow(block, from, to)).toList();
    List<ResolvedAvailabilityWindowResponse> externalBusyWindows =
        resolveExternalBusyWindows(template, from, to);

    List<ResolvedAvailabilityWindowResponse> recurringWindows =
        recurringBlockRepository
            .findByTemplateActiveInDateRange(template, fromDate, toDate)
            .stream()
            .flatMap(
                block -> expandRecurringBlock(block, zoneId, from, to, fromDate, toDate).stream())
            .toList();
  ...
  private List<ResolvedAvailabilityWindowResponse> subtractOneOffOverlaps(
      ResolvedAvailabilityWindowResponse recurringWindow,
      List<ResolvedAvailabilityWindowResponse> oneOffWindows) {
    List<OffsetRange> segments =
        new ArrayList<>(
            List.of(new OffsetRange(recurringWindow.getStartsAt(), recurringWindow.getEndsAt())));
    ...
    for (ResolvedAvailabilityWindowResponse oneOffWindow : oneOffWindows) {
      ...
      for (OffsetRange segment : segments) {
        if (!overlaps(
            segment.startsAt(),
            segment.endsAt(),
            oneOffWindow.getStartsAt(),
            oneOffWindow.getEndsAt())) {
  ...
  private List<ResolvedAvailabilityWindowResponse> expandRecurringBlock(
  ...
  private List<ResolvedAvailabilityWindowResponse> expandYearlyRecurringBlock(
      AvailabilityTemplateRecurringBlock block,
      ZoneId zoneId,
      OffsetDateTime from,
      OffsetDateTime to,
  ...
  private List<ResolvedAvailabilityWindowResponse> expandMonthlyRecurringBlock(
      AvailabilityTemplateRecurringBlock block,
      ZoneId zoneId,
      OffsetDateTime from,
      OffsetDateTime to,
    ...
    List<ResolvedAvailabilityWindowResponse> windows = new ArrayList<>();
    long occurrenceIndex = 0;
    for (YearMonth month = anchorMonth;
        !month.isAfter(effectiveEndMonth);
  ...
  private List<ResolvedAvailabilityWindowResponse> expandWeeklyRecurringBlock(
      AvailabilityTemplateRecurringBlock block,
      ZoneId zoneId,
      OffsetDateTime from,
      OffsetDateTime to,
    ...
    int interval = block.getIntervalCount();
    long firstIndex =
        Math.max(0, ceilingDiv(ChronoUnit.WEEKS.between(anchorDate, fromDate), interval));
  ...
  private List<ResolvedAvailabilityWindowResponse> expandDailyRecurringBlock(
      AvailabilityTemplateRecurringBlock block,
      ZoneId zoneId,
      OffsetDateTime from,
      OffsetDateTime to,
    ...
    int interval = block.getIntervalCount();
    long firstIndex =
        Math.max(0, ceilingDiv(ChronoUnit.DAYS.between(anchorDate, fromDate), interval));
  ...
  private void addRecurringOccurrence(
  ...
  private ResolvedAvailabilityWindowResponse toResolvedOneOffWindow(
      AvailabilityTemplateBlock block, OffsetDateTime from, OffsetDateTime to) {
    return ResolvedAvailabilityWindowResponse.builder()
