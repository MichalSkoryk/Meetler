...
class GroupEventControllerTest {
  ...
  @Test
  void exportToGoogleDelegatesToExportService() {
    UUID groupId = UUID.randomUUID();
    UUID eventId = UUID.randomUUID();
    AppUser user = user();
    ExportGroupEventResponse serviceResponse =
        ExportGroupEventResponse.builder().groupEventId(eventId).provider("GOOGLE").build();
    when(googleGroupEventExportService.exportToGoogle(groupId, eventId, user))
        .thenReturn(serviceResponse);

    ResponseEntity<ExportGroupEventResponse> response =
        controller.exportToGoogle(groupId, eventId, user);

    assertThat(response.getStatusCode().value()).isEqualTo(200);
    assertThat(response.getBody()).isSameAs(serviceResponse);
    verify(googleGroupEventExportService).exportToGoogle(groupId, eventId, user);
  }
